...
def test_list_trajets_default_pagination():
...
def test_list_trajets_filters_by_country_and_train_type():
    response = client.get(
        "/trajets",
...
def test_list_trajets_filters_by_origin_and_destination_text():
    response = client.get(
        "/trajets",
...
def test_list_trajets_rejects_inconsistent_distance_bounds():
    response = client.get(
        "/trajets",
...
def test_validation_errors_use_homogeneous_payload():
...
def test_get_trajet_by_stable_id_matches_list_item():
...
def test_get_trajet_returns_404_for_unknown_id():
...
def test_stats_volumes_exposes_global_and_grouped_counts():
    response = client.get("/stats/volumes")

    assert response.status_code == 200
    payload = response.json()
    assert payload["total_trajets"] > 0
    ...
    assert {item["key"] for item in payload["by_country"]} == {"DE", "ES", "FR", "IT"}
...
def test_health_includes_dataset_models_and_timestamp():
    ...
    assert "trip_id" in payload["dataset"]["columns"]
